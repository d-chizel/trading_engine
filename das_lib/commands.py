...
class CmdAPI:
    ...
    def short_sell_join_offer_for_all_gapped_stocks(self, connection, ask_buffer, autorun):
        for index, row in self.ticker_df.iterrows():
            if row['pre_trade_check_passed']:  # Only place short sell order if locate is available or already shortable
                symbol = row['ticker']
                shares_to_short = row['located_shares'] - row['shares_in_position'] if row['locate_available'] and row['shares_in_position'] else row['shares_to_short']
                route = "XALL"
                tif = "DAY"
    ...
    def inquire_short_locate_for_all_gapped_stocks(self, connection):
        for index, row in self.ticker_df.iterrows():
            ...
            self.ticker_df.at[index, 'total_locate_cost'] = 10001
            self.ticker_df.at[index, 'locate_price'] = 10001
    ...
    def short_locate_price_inquire_lowest(self, connection, symbol, shares_to_locate):
        locate_routes = ["ATLAS2", "ATLAS1", "ATLAS3", "ATLAS6", "ATLAS5"]
